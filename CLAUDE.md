...
- 모든 응답은 한글로 제공
- 스펙 문서 작성 시 반드시 한글로 작성
- 작업 진행 과정, 분석 결과, 리뷰 보고서 등 모든 산출물도 한글로 작성
- 코드 리뷰, 분석 문서, 검증 보고서 등의 내용도 한글 사용
- 코드 주석도 한글 사용
- 커밋 내용에도 한글 사용
- 기술 용어는 영문 병기 가능 (예: "트랜잭션(Transaction)")
- 구현 완료 후 자세한 설명을 제공

### 1.2 작업 시작 규칙

- IDE 에서 파일의 최신 상태를 확인 후 작업 진행
- 개발은 실제로 개발을 시작하라는 요청 시에 시작 - 키워드: 작성, 개발, 시작 등
- 계획은 실제로 계획을 시작하라는 요청 시에 시작 - 키워드: 계획, 플랜 등
- 그 외에는 일반 질문으로 간주
- 불명확한 사항이 있으면 작업을 중단하고 질문을 통해 명확히 함

### 1.3 문서 및 Steering 관리

- 질의나 task 진행 중 생성된 분석/리뷰 문서는 `docs/` 폴더에 위치
- rule 파일 작성/수정 시 간략히 핵심 내용만 작성
- 불필요한 설명이나 예시는 최소화
- 실행 가능한 규칙 위주로 작성
...
- 현재 프로젝트의 기술 스택 내에서 해결책을 우선적으로 고려
- 복잡하거나 어려운 기술은 최후의 수단으로만 시도
- 각 프로젝트에 맞는 수정사항을 알맞게 적용
- 중복 구현을 최소화하고 가능한 공통화 시도
- 필요한 기능만 우선 구현하고 부차적인건 승인 후 구현

### 2.2 파일 관리

- 기존 파일 수정을 우선 고려, 신규 파일 생성은 최후 수단
- 새로운 파일을 생성하기 전에 기존 파일의 명칭을 바꾸는 수정이 가능하다면 수정하는 쪽을 먼저 고려
- 새로운 파일을 생성하기 전에 기존 파일에서 구현 가능하다면 수정하는 쪽을 먼저 고려
- 한 파일에 여러 변경사항이 있으면 한 번에 수정
- 개발이 완료된 후 통합 가능 여부를 확인하고 불필요하게 된 파일은 제거
...
- 하드코딩된 설정값 사용 금지. (application.yml 또는 공통 코드 활용)
- 
- 과도한 이모지, 이모이콘 사용 금지

### 2.4 코드 검증

- 이번 수정으로 인해 기존의 다른 기능에 영향이 생겼는지 확인
- 요구사항을 모두 만족하면서 불필요한 개발이 없었는지 확인
- 성능 측면에서 문제가 발생할 수 있는지 확인
- 보안 측면에서 문제가 발생할 수 있는지 확인
