...
export class PaymentRetryService {
  ...
  async processPaymentWithRetry(
    ...
    try {
      ...
      if (retryState.attempts < retryConfig.maxRetries) {
        ...
        this.logger.log(`Payment ${paymentId} scheduled for retry #${retryState.attempts} in ${delay}ms`);
        
        throw new Error(`결제가 실패했습니다. ${Math.ceil(delay / 1000)}초 후 자동으로 재시도됩니다`);
      } else {
        await this.markPaymentAsFailed(paymentId, 'Max retries exceeded after failures');
        throw new Error('결제 처리에 실패했습니다. 다른 결제 수단을 이용해주세요');
      }
    }
  }
  ...
  async processPendingRetries(): Promise<void> {
    ...
    this.logger.log(`Processing ${pendingRetries.length} pending payment retries`);
  ...
  private async saveRetryState(paymentId: string, state: RetryState): Promise<void> {
    await this.cacheService.set(`payment:retry:${paymentId}`, state, { ttl: 86400 }); // 24 hours
  }
  ...
  private async scheduleRetry(
    ...
    setTimeout(async () => {
      try {
        ...
        this.logger.error(`Scheduled retry failed for payment ${paymentId}:`, error);
      }
    }, delay);
  }
  ...
  private recordFailure(provider: string): void {
    ...
    if (state.failures >= this.circuitBreakerThreshold) {
      ...
      this.logger.warn(`Circuit breaker opened for ${provider} after ${state.failures} failures`);
    }
  }
}