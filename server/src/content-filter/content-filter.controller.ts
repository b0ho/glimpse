...
export class ContentFilterController {
  ...
  @ApiResponse({ status: 200, description: '검증 결과' })
  async validateImage(
    @Body() body: { imageUrl: string; context: 'PROFILE' | 'CHAT' | 'GROUP' },
  ) {
  ...
  async analyzeReport(
    @Body() body: { reportedContent: string; reportReason: string },
  ) {
  ...
  async addBannedWord(@Body() body: { word: string; category?: string }) {
